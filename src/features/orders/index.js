...
const Orders = () => {
  ...
  const getStatus = (status) => {
    return (
      <div className={`badge font-medium py-2 px-3 ${status === 'delivered' ? 'bg-green-500 text-white' : 'badge-ghost'}`}>
        {status}
      </div>
    );
  };
  ...
  return (
    <>
      <TitleCard title={`${groupedOrders.length} Đơn hàng đã được giao`} topMargin="mt-2">
